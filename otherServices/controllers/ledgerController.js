...
exports.createLedger = async (req, res) => {
    ...
    try {
        ...
        if (ledgers.length > 0) {
            return errorMid(
                400,
                `Ledger with accountName: ${accountName} already exists`,
                req,
                res
            );
        }
        ...
        if (!findGroup) {
            if (!findGroup)
                return errorMid(
                    400,
                    `Group with groupSrNo: ${groupSrNo} not found`,
                    req,
                    res
                );
        }
        ...
        if (!["DR", "CR"].includes(op_acc_DR_CR))
            return errorMid(
                400,
                `op_acc_DR_CR: ${op_acc_DR_CR} not valid Please provide [DR, CR]`,
                req,
                res
            );
        ...
        if (typeof op_acc_balance !== "number")
            return errorMid(
                400,
                `op_acc_balance: ${op_acc_balance} not valid Please provide number`,
                req,
                res
            );
        ...
        if (!["DR", "CR"].includes(ly_acc_DR_CR))
            return errorMid(
                400,
                `ly_acc_DR_CR: ${ly_acc_DR_CR} not valid Please provide [DR, CR]`,
                req,
                res
            );
        ...
        if (typeof ly_cl_balance !== "number")
            return errorMid(
                400,
                `ly_cl_balance: ${ly_cl_balance} not valid Please provide number`,
                req,
                res
            );
        ...
        if (typeof loan_deduct !== "boolean")
            return errorMid(
                400,
                `loan_deduct: ${loan_deduct} not valid Please provide [true, false]`,
                req,
                res
            );
        ...
        if (typeof exception_checking !== "boolean")
            return errorMid(
                400,
                `exception_checking: ${exception_checking} not valid Please provide [true, false]`,
                req,
                res
            );
        ...
        if (typeof exception_amount !== "number")
            return errorMid(
                400,
                `exception_amount: ${exception_amount} not valid Please provide number`,
                req,
                res
            );
    ...
    try {
        ...
        if (op_acc_DR_CR) {
            if (!["DR", "CR"].includes(op_acc_DR_CR))
                return errorMid(
                    400,
                    `op_acc_DR_CR: ${op_acc_DR_CR} not valid`,
                    req,
                    res
                );
        ...
        if (op_acc_balance) {
            if (typeof op_acc_balance !== "number")
                return errorMid(
                    400,
                    `op_acc_balance: ${op_acc_balance} not valid`,
                    req,
                    res
                );
        ...
        if (ly_acc_DR_CR) {
            if (!["DR", "CR"].includes(ly_acc_DR_CR))
                return errorMid(
                    400,
                    `ly_acc_DR_CR: ${ly_acc_DR_CR} not valid`,
                    req,
                    res
                );
        ...
        if (ly_cl_balance) {
            if (typeof ly_cl_balance !== "number")
                return errorMid(
                    400,
                    `ly_cl_balance: ${ly_cl_balance} not valid`,
                    req,
                    res
                );
        ...
        if (loan_deduct_amount) {
            if (typeof loan_deduct_amount !== "number")
                return errorMid(
                    400,
                    `loan_deduct_amount: ${loan_deduct_amount} not valid`,
                    req,
                    res
                );
        ...
        if (exception_checking) {
            if (typeof exception_checking !== "boolean")
                return errorMid(
                    400,
                    `exception_checking: ${exception_checking} not valid`,
                    req,
                    res
                );
        ...
        if (exception_amount) {
            if (typeof exception_amount !== "number")
                return errorMid(
                    400,
                    `exception_amount: ${exception_amount} not valid`,
                    req,
                    res
                );
        ...
        const updatedLedger = await ledgerService.updateLedger(
            sr_no,
            dataForUpdate
        );
