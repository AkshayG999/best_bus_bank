...
const { memberNomineeModel, memberRelationModel, genderModel } = require("../../db/db");
...
exports.getAll = async (filter = {}) => {
    try {
        return await memberNomineeModel.findAll({
            where: filter,
            include: [{ model: memberRelationModel, as: "member_relation" }],
        });
    } catch (error) {
...
exports.getByMem_EntryNo = async (Mem_EntryNo) => {
    try {
        const nominee = await memberNomineeModel.findOne({
            where: { Mem_EntryNo },
            include: [
                {
                    model: memberRelationModel, as: "member_relation",
                }, {
...
exports.update = async (Mem_EntryNo, updateData, transaction = null) => {
    try {
        const options = transaction ? { transaction } : {};
        const [rowsUpdate, [updatedData]] = await memberNomineeModel.update(
            updateData,
            {
                where: { Mem_EntryNo },
                returning: true,
        ...
        if (rowsUpdate === 0)
            throw new Error(`No nominee found with EntryNo ${Mem_EntryNo}`);
...
exports.delete = async (Mem_EntryNo, transaction = null) => {
    try {
        ...
        if (deleted === 0) return null;        //throw new Error(`No nominee found with EntryNo ${Mem_EntryNo}`);
        return deleted;
    } catch (error) {
